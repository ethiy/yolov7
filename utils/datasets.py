...
def img2label_paths(img_paths, lazy = False):
    # Define label paths as a function of image paths
    if lazy:
        return [Path(img_path).parent / (Path(img_path).stem + ".txt") for img_path in img_paths]
    else:
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='', lazy = False):
        self.img_size = img_size
        ...
        # Check cache
        self.label_files = img2label_paths(self.img_files, lazy=lazy)  # labels
        ...
        self.label_files = img2label_paths(cache.keys(), lazy=lazy)  # update
        if single_cls:
...
def extract_boxes(path='../coco/', lazy = False):  # from utils.datasets import *; extract_boxes('../coco128')
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in img_formats:
            ...
            # labels
            lb_file = Path(img2label_paths([str(im_file)], lazy=lazy)[0])
...
def autosplit(path='../coco', weights=(0.9, 0.1, 0.0), annotated_only=False, lazy = False):
    ...
    for i, img in tqdm(zip(indices, files), total=n):
        if not annotated_only or Path(img2label_paths([str(img)], lazy=lazy)[0]).exists():  # check label
